...
class ConvNetTrain:
    ...
    def load_dataset(self, path, ratio=1.0):
        x, y = [], []
            ...
            if os.path.exists(imgs_dir):
                for img_file in os.listdir(imgs_dir):
                    ...
                    if img is not None:
                        ...
                        y.append(label)
                        cnt += 1
                        if cnt == int(ratio*self.LBL_MAX[label]):
                            break
        x = np.asarray(x)
        y = np.asarray(y)
        return x, y

    def prepare_data(self, ratio=1.0):
        train_path = os.path.join(self.data_path, 'train')
        test_path = os.path.join(self.data_path, 'test')
        self.X_train, self.y_train = self.load_dataset(train_path, ratio=ratio)
    ...
    def train(self, ratio=1.0):
        self.prepare_data(ratio=ratio)
    ...
    def summary(self):
        ...
        stats = pd.DataFrame()
        columns = ["p", "n", "tp", "tn", "fp", "fn", "tpr", "tnr", "sensivity", "specifity", "acc", "f1_score"]
